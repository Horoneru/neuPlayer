...
{
public:
  NcWidgetData(NcFramelessHelperImpl* _d, QWidget* topLevelWidget , bool alwaysOnTop);
  ~NcWidgetData();
...
NcWidgetData::NcWidgetData( NcFramelessHelperImpl* _d, QWidget* topLevelWidget, bool alwaysOnTop )
{
  d = _d;
  ...
  if(alwaysOnTop)
      mWidget->setWindowFlags( Qt::CustomizeWindowHint|Qt::WindowStaysOnTopHint|Qt::FramelessWindowHint);
  else
                                   ...
                                   bool borders, bool animatedEntrance)
{
  ...
  NcWidgetData* data = new NcWidgetData( d, topLevelWidget, alwaysOnTop );

  if(borders)
      setCustomBorders(topLevelWidget);
  ...
  if(animatedEntrance)
  {
      ...
      QPointer <FadeWindow> fadeIn = new FadeWindow((QMainWindow*)topLevelWidget, 400, FadeWindow::FadeIn, this);
      fadeIn->start(settings.value("opacity", 1.0).toReal());
  }

}

void NcFramelessHelper::setCustomBorders(QWidget *topLevelWidget)
{
    QRegion region;

    // middle and borders
    region += topLevelWidget->rect().adjusted(6, 0, -6, 0);
    region += topLevelWidget->rect().adjusted(0, 6, 0, -6);

    // top left
    QRect corner(topLevelWidget->rect().topLeft(), QSize(6*2, 6*2));
    region += QRegion(corner, QRegion::Ellipse);

    // top right
    corner.moveTopRight(topLevelWidget->rect().topRight());
    region += QRegion(corner, QRegion::Ellipse);

    // bottom left
    corner.moveBottomLeft(topLevelWidget->rect().bottomLeft());
    region += QRegion(corner, QRegion::Ellipse);

    // bottom right
    corner.moveBottomRight(topLevelWidget->rect().bottomRight());
    region += QRegion(corner, QRegion::Ellipse);
    topLevelWidget->setMask(region);
}
